...
class PolynomialTable {
	...
	xor(a, mod = 256) {
		...
		let sum = 0
		a.forEach(b => sum ^= this.log(b))
		return this.alog(sum % mod)
	}
	mult(a, b) {
		const c = {}
	...
	div(a, b) {
		...
		for (let i = 0; i < a.length; i++) {
			...
			const term = this.alog(aa[0])
			const c = []
			for (let j = 0; j < bb.length; j++) {
				c[j] = bb[j] ? this.log(this.mod(bb[j] + term)) : 0
			}
...
export default new PolynomialTable()